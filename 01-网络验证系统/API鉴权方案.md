...
| **格式**         | JWT (HS256 签名)                                                      |
| **有效期**        | **15 分钟**                                                           |
| **Payload 内容** | `user_id`, `user_level`, `game_project_id`（可选）, `exp`, `iat`, `jti` |
...
project 为可选字段，若用户只有一个游戏授权则可省略

### 2.2 Refresh Token
用于获取新的 Access Token，实现免密续期。

属性	值
格式	随机字符串（UUID v4 + 哈希）或 JWT（推荐用不透明字符串）
...
### 3.1 首次登录
text
客户端 → POST /api/v1/auth/login {username, password, device_fingerprint}
服务端：
  1. 校验用户名密码
  2. 校验用户状态、授权有效性
  3. 生成 Access Token (AT) 和 Refresh Token (RT)
  4. 将 RT 存入 Redis，关联 AT 的 jti
  5. 返回 { access_token, refresh_token, expires_in }
客户端：
  - PC/安卓：安全存储 RT，AT 存内存
  - Web：RT 存入 HttpOnly Cookie，AT 存内存（或也存 Cookie）
### 3.2 请求鉴权
text
...
滚动刷新策略：
每次使用 RT 刷新 AT 时，可选择是否同时发放新的 RT（旧 RT 立即失效）。这能提高安全性，但需要客户端支持同时更新两个 Token。Phase 1 暂不实施滚动刷新，RT 在有效期内可多次使用。

### 3.4 登出
text
客户端 → POST /api/v1/auth/logout (需携带 AT)
服务端：
...
POST	/api/v1/auth/logout	登出	AT
POST	/api/v1/auth/refresh	刷新 Token	RT
POST	/api/v1/auth/revoke-all	踢出所有设备	AT
GET	/api/v1/auth/me	获取当前用户信息	AT
## 八、待办事项 / 后续推演
确定 JWT 签名算法：HS256 vs RS256（初期 HS256 足够，后期可切 RS256）

设计设备指纹生成规则（安卓端获取 IMEI/AndroidID 的替代方案）
