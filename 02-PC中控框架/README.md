...
- 本目录当前作为 PC 中控框架的 Obsidian 文档入口。
- 本轮目录级治理只检查文档口径，不重新复核 HiveGreatSage-PCControl 当前 main 源码。
- README 中的"已创建 / 已封装 / 已确认"表示历史文档登记或当时整理结论，不代表当前 R1 / E2 / M1 级闭环。
- 涉及发布、联调、生产可用判断时，必须回到 GitHub main 源码、测试运行和实际 PC + 安卓设备验证。

本目录存放 [[00-项目总控/项目总大纲#三、子系统2：PC 中控框架]] 的详细设计文档。
...
├── PC安卓局域网通信方案.md             ← 已创建
├── 跨账号智能调度分析.md               ← 已创建
├── 通信协议.md                        ← 后续可选立项；当前由 PC安卓局域网通信方案 承接，不在本轮创建
├── ui设计/                            ← 已存在，UI 规范草稿入口
├── ui设计/界面规范.md                  ← 已存在
└── 打包与发布.md                      ← 后续可选立项；不在本轮创建
```

## 当前登记结论

以下为历史文档登记结论，未在本轮重新做源码级复核。
