...
关联文档:
  ...
  - "[[05-工具与拓展/ADB+CV提取工具]]"
变更记录:
  - V1.0.1 (2026-05-12): 收口 ADB 管理模块证据边界；修正正文旧定稿口径；明确历史设计不代表当前源码、打包或实机验证闭环
  - V1.0.0: Obsidian 去漂移重构生成
---

# ADB 管理模块

> **当前定位：** 本文是 PC 中控 ADB 管理模块的设计草稿入口。
> **历史说明：** 本文保留早期 ADB 模块设计内容，原始设计来自历史 AI 对话记录。
> **证据边界：** 文中的"已定稿"表示历史设计口径，不代表当前 GitHub main 源码、打包路径、ADB 实机连接或批量激活已完成验证。
> **关联文档：** [[02-PC中控框架/架构设计]] · [[05-工具与拓展/ADB+CV提取工具]]
...
- 本文当前作为 D1 级文档证据维护。
- 本轮未重新逐行复核 PCControl 的 `core/utils/adb_manager.py`。
- 本轮未运行 ADB 连接测试。
- 本轮未完成 PyInstaller / Nuitka 打包路径验证。
- 本轮未完成真实安卓设备 USB / TCP/IP / ROOT 激活验证。
- 不得把本文中的"已定稿"直接写成"已实现 / 已测试 / 可发布"。
...
|项目|风险等级|说明|
|---|---|---|
|PyInstaller 打包后路径|⚠️ 中|`_MEIPASS` 临时目录路径需实测|
|守护进程重启后是否存活|⚠️ 中|`--daemon` 重启不一定自动恢复|
|厂商 ROM `/data/local/tmp` 权限|⚠️ 中|部分机型可能拒绝写入或执行|
