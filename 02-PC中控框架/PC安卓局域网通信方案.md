...
关联文档:
  ...
  - "[[03-安卓脚本框架/README]]"
变更记录:
  - V1.0.1 (2026-05-12): 目录级治理；移除重复旧元数据块；收口 PC↔安卓局域网通信方案证据边界；统一状态符号和检查清单格式
  - V1.0.0: Obsidian 去漂移重构生成
---

# PC ↔ 安卓局域网通信方案设计

> **当前定位：** 本文是 PC 中控与安卓脚本局域网通信的方案草稿入口。
> **历史说明：** 本文保留早期基于懒人精灵网络能力整理出的通信方案。
> **证据边界：** 文中的"已锁定 / 最终方案 / 当前 MVP"均表示历史设计口径，不代表当前 PCControl / AndroidScript 源码、联调或实机验证已完成。
> **关联文档：** [[02-PC中控框架/架构设计]] · [[03-安卓脚本框架/README]]
...
- 本文当前作为 D1 级方案文档证据维护。
- 本轮未重新复核 PCControl WebSocket 服务端源码。
- 本轮未重新复核 AndroidScript WebSocket 客户端源码。
- 本轮未完成 PC ↔ 安卓局域网联调。
- 本轮未完成真实工作室网络环境验证。
- 不得把本文中的方案设计写成"已实现 / 已联调 / 已闭环"。
